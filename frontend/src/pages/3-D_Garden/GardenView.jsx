...
const GardenView = () => {
  ...
  const PAINTING_DETAILS = {
    ...
    "Figure on Rickshaw": {
      artist: "Tyeb Mehta",
      year: "1984",
      description: "This powerful painting is part of Tyeb Mehta's iconic Rickshaw Puller series, depicting a figure merged with a rickshaw in his distinctive minimalist style. The work captures the struggle and dignity of urban labor, showing a human figure geometrically fragmented and integrated with the rickshaw, symbolizing the dehumanizing aspects of poverty and labor. The stark color planes and diagonal compositions create a sense of tension and movement, while the distorted human form reflects the physical and emotional burden of the rickshaw puller's existence. This painting represents Mehta's recurring theme of human suffering and resilience in modern India.",
      technique: "Oil on canvas",
      location: "Private Collection",
      style: "Modern Indian Art, Minimalist Expressionism",
      significance: "Part of Mehta's significant Rickshaw series that emerged from his experiences in Kolkata, representing his artistic response to urban poverty and human condition"
    },
    "The Last Supper": {
      artist: "Leonardo da Vinci",
      year: "1495-1498",
    ...
    "Girl with a Pearl Earring": {
      ...
      year: "1665",
      description: "Often called the 'Mona Lisa of the North', this masterpiece depicts a young woman in exotic dress, looking over her shoulder with an enigmatic expression. The painting is renowned for its intimate portrayal and the luminous quality of light, particularly in the girl's gaze and the lustrous pearl earring. The subject's identity remains unknown, adding to the painting's mystery. The exotic turban and the dramatic use of light against a dark background creates a timeless and mesmerizing effect that has captivated viewers for centuries.",
      technique: "Oil on canvas",
    ...
    "Departure": {
      artist: "Krishen Khanna",
      year: "2008",
      description: "Departure is a powerful composition that captures the essence of human movement and transition. The painting depicts a group of figures in motion, rendered in Khanna's distinctive style that combines gestural brushwork with emotional depth. The work shows people in various states of movement, their forms intertwined and overlapping, creating a sense of collective journey. The muted color palette, dominated by earthy tones and dramatic contrasts, adds to the painting's emotional resonance. This piece reflects Khanna's recurring theme of human displacement and migration, drawing from his personal experiences of Partition and his observations of urban life in India.",
      technique: "Oil on canvas",
      location: "DAG Modern Art Gallery",
      style: "Contemporary Indian Art, Modernism",
      significance: "The painting is part of Khanna's broader exploration of human movement and displacement in modern India. It represents his mature style where figurative elements merge with abstract expressionist techniques.",
      additionalInfo: "Krishen Khanna, born in 1925, is one of India's most significant modernist painters. His work often deals with themes of migration, daily life, and human relationships, influenced by his experiences during the Partition of India and his observations of urban life."
    }
  };
...
<div id="paintingPopup" style="display: none; position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); 
  ...
  function createSpace(config) {
    ...
    const grassNormalURL = 'https://threejs.org/examples/textures/terrain/grasslight-big-nm.jpg';

    // Floor with grass texture + normal map
    const floorGeometry = new THREE.PlaneGeometry(config.width, config.depth);

    const loader = new THREE.TextureLoader();
    const grassTexture = loader.load(grassURL);
    const grassNormal = loader.load(grassNormalURL);

    // Tile the texture for detail
    grassTexture.wrapS = grassTexture.wrapT = THREE.RepeatWrapping;
    grassNormal.wrapS = grassNormal.wrapT = THREE.RepeatWrapping;
    grassTexture.repeat.set(config.width / 5, config.depth / 5);
    grassNormal.repeat.set(config.width / 5, config.depth / 5);
    ...
    const treeTexture = new THREE.TextureLoader().load('https://cdn.pixabay.com/photo/2023/05/12/15/22/tree-7988736_1280.jpg');
    treeTexture.wrapS = THREE.RepeatWrapping;
    treeTexture.wrapT = THREE.RepeatWrapping;
    treeTexture.repeat.set(10, 1);

    const wallMaterial = new THREE.MeshStandardMaterial({ map: treeTexture });
  ...
  function setupLighting() {
    ...
    // Add sky gradient to enhance blue sky effect
    createSkyGradient();
  }

  function createSkyGradient() {
    // Create a sky gradient with THREE.GradientTexture
    const canvas = document.createElement('canvas');
    ...
    gradient.addColorStop(0.0, '#87CEEB');  // Sky blue
    gradient.addColorStop(0.5, '#5D9BEC');  // Medium blue
    gradient.addColorStop(1.0, '#1E90FF');  // Deep sky blue
    ...
    if (painting && !popupVisible) {
      const indicatorHTML = `
            <div id="paintingIndicator" style="position: fixed; bottom: 20px; left: 50%; 
                ...
                <div style="font-size: 14px; color: #ffd700; width: 200px;">Press SPACE to view details</div>
            </div>`;
  ...
  function createFlower(type = "red") {
    const group = new THREE.Group();
    ...
    if (type === "red") {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        petal.position.set(Math.cos((i / 5) * Math.PI * 2) * 0.2, 1, Math.sin((i / 5) * Math.PI * 2) * 0.2);
        group.add(petal);
      }
    }
    else if (type === "white") {
      ...
      for (let i = 0; i < 8; i++) {
        const petal = new THREE.Mesh(petalGeometry, petalMaterial);
        petal.position.set(Math.cos((i / 8) * Math.PI * 2) * 0.25, 1, Math.sin((i / 8) * Math.PI * 2) * 0.25);
        group.add(petal);
      }
      const center = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), new THREE.MeshStandardMaterial({ color: 0xffff00 }));
      center.position.y = 1;
      group.add(center);
    }
      ...
      const petalMaterial = new THREE.MeshStandardMaterial({ color: 0xffff00 });
      for (let i = 0; i < 10; i++) {
        const petal = new THREE.Mesh(petalGeometry, petalMaterial);
        petal.position.set(Math.cos((i / 10) * Math.PI * 2) * 0.3, 1, Math.sin((i / 10) * Math.PI * 2) * 0.3);
        group.add(petal);
      }
      const center = new THREE.Mesh(new THREE.SphereGeometry(0.15, 8, 8), new THREE.MeshStandardMaterial({ color: 0x8B4513 }));
      center.position.y = 1;
      group.add(center);
    }
    else if (type === "bush") {
      ...
      for (let i = 0; i < 3; i++) {
        const sphere = new THREE.Mesh(new THREE.SphereGeometry(0.4, 8, 8), bushMaterial);
        sphere.position.set((Math.random() - 0.5) * 0.5, 0.4 + Math.random() * 0.2, (Math.random() - 0.5) * 0.5);
        group.add(sphere);
      }
    }
  ...
  // Add them randomly
  function addFlowersAndPlants() {
    const types = ["red", "white", "sunflower", "bush"];
    for (let i = 0; i < 30; i++) {
      const type = types[Math.floor(Math.random() * types.length)];
      const flower = createFlower(type);
      flower.position.set(
        (Math.random() - 0.5) * ROOM_CONFIG.mainHall.width * 0.8,
        0,
        (Math.random() - 0.5) * ROOM_CONFIG.mainHall.depth * 0.8
      );
      scene.add(flower);
    }
  }
